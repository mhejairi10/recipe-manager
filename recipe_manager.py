...
def isValidInputNumber(target):

    """
    
    Check whether the target is digit(s).

    Args:
       isValidInputNumber(target)

    Returns:
        True if target is digit, otherwise False
...
    Member:
    ...
    # withotu bool return "None" if not valid, for tranform it to false use bool() option1:
    return bool(re.fullmatch(pattern, target.strip()))
    #option2:
    #return target.strip().isdigit()
    
def isValidDifficulty(difficulty):
...
def isValidCookedChoice(cooked):
    valid_choices = ["Yes", "No", "Today"]
    return cooked in valid_choices


def isValidDate(cooked_date):
    try:
        datetime.strptime(cooked_date, "%Y-%m-%d")
        return True
    except ValueError:
        return False
